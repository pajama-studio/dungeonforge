...
#   ./scripts/pipeline-court-expansion.sh --catalog  # also write the catalogue
set -euo pipefail
...
for dir in "$SRC_ROOT"/*/; do
  ...
  ok=1
  for pair in "lod0-30000tri:render-30k:1024" "lod1-8000tri:render-8k:512" "lod2-2500tri:render-2500:256"; do
    lod="${pair%%:*}"; rest="${pair#*:}"; name="${rest%%:*}"; tex="${rest##*:}"
    in=$(ls "$WORK/$slug"/*"$lod"*.glb 2>/dev/null | head -1)
